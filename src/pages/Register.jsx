...
export default function Register(){
  ...
  async function handleSubmit(e){
    ...
    setError('')
    try{
      const res = await api.post('/customer/auth/registration', {
      ...
      if(token){
        ...
        alert(res.data?.message || 'Registration complete. You can now login.')
        navigate('/login')
      }
    }catch(err){
    ...
    <div className="min-h-screen flex items-center justify-center py-20 px-6">
      <div className="w-full max-w-2xl">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">Join thousands moving with ohcargo</p>
        </div>
        ...
        <div className="card p-8">
          ...
          <div className="mt-6 text-center">
            <p className="text-gray-600">Already have an account? <a href="/login" className="text-primary-600 hover:text-primary-700 font-semibold">Sign in</a></p>
          </div>
        </div>
      </div>
    </div>
